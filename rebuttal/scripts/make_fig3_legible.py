...
OUT = ROOT / "rebuttal" / "figures_legible"
...
def panel_main_rank(ax: plt.Axes, gate: dict, rank_stats: pd.DataFrame) -> pd.DataFrame:
    ...
    ax.set_title("Rank-1 gate across 30 blocks", loc="left", fontsize=10.65, pad=4)
    ax.set_xlim(0, 23.5)
    ...
    for method, label in callouts.items():
        ...
        ax.text(17.0, idx, label, va="center", fontsize=7.95, color=color)
...
def panel_score(ax: plt.Axes, metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    ax.set_title("Utility at matched gate protocol", loc="left", fontsize=10.65, pad=4)
    ax.grid(axis="x", color="#ECECEC", linewidth=0.45)
    lighten_spines(ax)
    return plot_df[
        [
...
def panel_k_sweep(ax: plt.Axes, r039: pd.DataFrame, r040: pd.DataFrame) -> pd.DataFrame:
    ...
    ax.text(4.46, sil["candidate_avg_rank"] + 0.1, "silhouette fails", fontsize=7.81, color=PALETTE["red"])
    ax.text(
        2.05,
        3.45,
    ...
    ax.set_title("k sensitivity and unsupervised selection", loc="left", fontsize=10.65, pad=4)
...
def panel_feature_ablation(ax: plt.Axes, r040: pd.DataFrame) -> pd.DataFrame:
    ...
    ax.set_title("Feature-family ablation", loc="left", fontsize=10.65, pad=4)
    ax.grid(axis="x", color="#ECECEC", linewidth=0.45)
    lighten_spines(ax)
    return plot_df[
        [
...
"""
# Regenerated Figure 3 with legible type (R1/R2/R3 figure-size comments).
# Derived from scripts/make_nature_workload_figures.py: base font 6.7 -> 9.0 pt, every
# explicit fontsize x1.42, canvas 7.2x5.6 -> 7.2x4.9 in, so the panel can be included at
# full two-column width without pushing the paper onto an extra page.
